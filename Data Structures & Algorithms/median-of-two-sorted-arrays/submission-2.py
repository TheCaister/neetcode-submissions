...
class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        first_list = [float('-inf')]
        first_list.extend(nums1)
        first_list.append(float('inf'))
        second_list = [float('-inf')]
        second_list.extend(nums2)
        second_list.append(float('inf'))
        
        total_len = len(first_list) + len(second_list)
        l, r = 0, len(first_list) - 1
        ...
        while l <= r:
            ...
            if nums2_len > 0 and second_list[nums2_len - 1] > first_list[nums1_len]:
                l = mid + 1
            elif first_list[mid] > second_list[nums2_len]:
                r = mid - 1
            else:
                if total_len % 2 == 0:
                    return (max(first_list[nums1_len - 1], second_list[nums2_len - 1]) + min(first_list[nums1_len], second_list[nums2_len])) / 2
                else:
                    return min(first_list[nums1_len], second_list[nums2_len])
